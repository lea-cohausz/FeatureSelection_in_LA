...
from iamb_based_dag import mb_exdesc_iamb, exdesc_iamb

def experiment_sap(data, n_splits=5, random_state=42):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        X_train_sensitive = X_train["ge"] #needs to be adjusted for other datasets
        X_test_sensitive = X_test["ge"]  # needs to be adjusted for other datasets
        ...
        for fs_name in feature_selectors:
            if fs_name == 'SelectKBest_f_classif20':
                ...
                if not descendants:
                    X_train_fs = X_train.drop("ge", axis=1)
                    X_test_fs = X_test.drop("ge", axis=1)
                    y_train_np = y_train.values
                    y_test_np = y_test.values
                else:
                    X_train_fs = X_train.drop(descendants, axis=1)
                    X_test_fs = X_test.drop(descendants, axis=1)
                    if "ge" not in descendants:
                        X_train_fs = X_train_fs.drop(["ge"], axis=1)
                        X_test_fs = X_test_fs.drop(["ge"], axis=1)
                    y_train_np = y_train.values
                    y_test_np = y_test.values
                ...
                X_train_fs, X_test_fs = my_fair_lady(X_train, X_test, ["ge"], dominant_data_type="discrete")
                y_train_np = y_train.values
                y_test_np = y_test.values
                ...
                X_train_fs, X_test_fs = disparate_impact(X_train, X_test, y_train_binary, y_test_binary, 'ge', label_name='target', repair_level=1.0)
                y_train_np = y_train.values
                y_test_np = y_test.values
            elif fs_name == 'Correlation_Remover':
                X_train_fs, X_test_fs = correlation_remover(X_train, X_test, ["ge"], alpha=1.0)
                y_train_np = y_train.values
                y_test_np = y_test.values
